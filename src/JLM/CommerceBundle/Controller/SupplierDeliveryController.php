...
use JLM\CommerceBundle\Event\SupplierDeliveryEvent;
 ...
 */
class SupplierDeliveryController extends ContainerAware
{
	/**
	 * List bills
	 */
	...
	{
		$manager = $this->container->get('jlm_commerce.supplierdelivery_manager');
		$manager->secure('ROLE_OFFICE');
		
		return $manager->renderResponse('JLMCommerceBundle:SupplierDelivery:index.html.twig',
				$manager->pagination('getCountAll', 'getAll', 'supplierDelivery')
		);
	}
    ...
    public function showAction($id)
    {
        ...
        return $manager->renderResponse('JLMCommerceBundle:SupplierDelivery:show.html.twig',array('entity'=> $manager->getEntity($id)));
    }
    ...
    public function newAction(Request $request)
    {
    	$manager = $this->container->get('jlm_commerce.supplierdelivery_manager');
    	$manager->secure('ROLE_OFFICE');
    	$form = $this->container->get('form.factory')->create('jlm_commerce.type.supplierdelivery');
    	$form->handleRequest($request);
		if ($form->isValid())
		{
			$entity = $form->getData();
			$manager->dispatch(JLMCommerceEvents::SUPPLIERDELIVERY_AFTER_PERSIST, new SupplierDeliveryEvent($entity, $manager->getRequest()));

			return $manager->redirect('supplierdelivery_show', array('id' => $form->getData()->getId()));
		}
		
        return $manager->renderResponse('JLMCommerceBundle:SupplierDelivery:new.html.twig', array(

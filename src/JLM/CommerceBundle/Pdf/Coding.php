    ...
    private function init()
    {
        ...
        foreach ($this->entity->getLines() as $line) {
            if (!$line->isService() && $line->getReference() !== 'TITLE' && $line->getReference() !== 'ST') {
                $denom = ($line->getPurchasePrice() * (1 - $line->getDiscountSupplier()) * ($line->getExpenseRatio()
                        + 1));
                ...
                $content[] = [
                    ...
                    number_format($line->getPurchasePrice() * $line->getDiscountSupplier(), 2, ',', ' ') . ' ' . chr(
                        128
                    ),
                    number_format($line->getPurchasePrice() * (1 - $line->getDiscountSupplier()), 2, ',', ' ')
                    . ' '
                    . chr(128),
                    ...
                    number_format(
                        ...
                        * (1 - $line->getDiscountSupplier())
                        * ($line->getExpenseRatio() + 1) + $line->getShipping(),
                        2,
                        ',',
                        ' '
                    ) . ' ' . chr(128),
                    number_format($p, 0, ',', ' ') . ' %',
                    number_format(
                        $line->getQuantity() * ($line->getUnitPrice() - ($line->getPurchasePrice() * (1
                                    - $line->getDiscountSupplier()) * ($line->getExpenseRatio() + 1)
                                + $line->getShipping())),
                        2,
                        ',',
                        ' '
                ...
                $totalpurchase += $line->getQuantity()
                    * $line->getPurchasePrice()
                    * (1 - $line->getDiscountSupplier())
                    * ($line->getExpenseRatio() + 1) + $line->getShipping();
                $totalsell += $line->getQuantity() * $line->getUnitPrice();
                $totalpurchaseFourniture += $line->getQuantity() * $line->getPurchasePrice() * (1
                        - $line->getDiscountSupplier());
                $fill[] = [
                    255,
                    255,
                    255,
        ...
        foreach ($this->entity->getLines() as $line) {
            if ($line->isService()) {
                $content[] = [
                    ...
                    number_format(
                        (($line->getUnitPrice() - $line->getShipping()) / ($line->getPurchasePrice() * (1
                                    - $line->getDiscountSupplier()) * ($line->getExpenseRatio() + 1)) - 1) * 100,
                        0,
                        ',',
                        ' '
                    ) . ' %',
        ...
        $content[] = [
            ...
            '',
            utf8_decode('reste'),
            number_format(($totalsell * (1 - $this->entity->getDiscount())) - $totalpurchase, 2, ',', ' ') . ' ' . chr(
                128
            ),
            'PV',

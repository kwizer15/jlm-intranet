	...
	, vatchange : function(e) {
		...
		$("#jlm_commerce_bill_lines tr").each(function() {
			if ($('#' + $(this).attr('id') + '_isTransmitter').val() == 0)
				...
				$('#' + $(this).attr('id') + '_vat').val($("#jlm_commerce_bill_vatTransmitter").val());
			$('#' + $(this).attr('id')).change();
		});
	}
	, newline : function(e){
	...
	BillLine.prototype = {
			...
			, listen : function() {
				...
				, select: function (event, ui) {
					...
					$(id + 'vat').val(($(id + 'isTransmitter').val() == '1') ? $("#jlm_commerce_bill_vatTransmitter").val() : $("#jlm_commerce_bill_vat").val());

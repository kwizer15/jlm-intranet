    ...
    public function editAction($id = 0)
    {
        ...
        if ($manager->getHandler($form)->process()) {
            ...
            if ($ajax) {
                ...
                $contact['contact']['contact']['show_link'] =  $router->generate('jlm_contact_contact_show', ['id' => $contact['contact']['id']]);
                ...
                $response = $manager->redirect('jlm_contact_contact_show', ['id' => $entity->getCorporation()->getId()]);
            }
        } else {
            $template = ($ajax) ? 'modal_new.html.twig' : 'new.html.twig';
            $response = $manager->renderResponse('JLMContactBundle:CorporationContact:' . $template, ['form'=>$form->createView()]);
        }
...
//          $manager->getSession()->setFlash('notice', $entity->getName().' a bien été supprimé');
        ...
        return $manager->renderResponse('JLMContactBundle:CorporationContact:' . $template, ['form'=>$form->createView()]);
    }
}
